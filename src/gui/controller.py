...
from src.core.autostart import set_auto_start, is_auto_start_enabled
...
class GatewayController:
    ...
    def _on_server_status(self, status: str):
        ...
        if status == "started":
            self._log(
                f"Gateway started on {self._config.settings.host}:{self._config.settings.port}",
                "success")
        elif status == "stopped":
    ...
    def add_channel(self, data: dict):
        """Add a new channel"""
        channel = ChannelConfig(id=self._config.next_channel_id(), **data)
    ...
    def delete_channel(self, channel_id: int):
        ...
        self._config.channels = [
            c for c in self._config.channels if c.id != channel_id
        ]
        self._save()
    ...
    def add_token(self, data: dict):
        """Add a new token"""
        token = TokenConfig(id=self._config.next_token_id(),
    ...
    def update_token(self, token_id: int, data: dict):
        ...
        for i, t in enumerate(self._config.tokens):
            if t.id == token_id:
                updated = TokenConfig(id=token_id,
                                      allowed_channels=data.pop(
                                          "allowed_channels",
                                          t.allowed_channels),
                                      **data)
    ...
    def delete_token(self, token_id: int):
        """Delete a token"""
        token = next((t for t in self._config.tokens if t.id == token_id),
                     None)
        name = token.name if token else str(token_id)
        self._config.tokens = [
            t for t in self._config.tokens if t.id != token_id
        ]
        self._save()
    ...
    def update_settings(self, data: dict):
        ...
        old_auto_start = self._config.settings.auto_start
        self._config.settings = GatewaySettings(**data)
        self._save()

        # Handle auto-start change
        if old_auto_start != self._config.settings.auto_start:
            self._apply_auto_start(self._config.settings.auto_start)
    ...
    def _apply_auto_start(self, enabled: bool):
        ...
        if set_auto_start(enabled):
            ...
            self._log(f"Auto-start on login {status}", "success")
        else:
            self._log("Failed to update auto-start setting", "error")
    ...
    def sync_auto_start_state(self):
        """Sync auto-start registry state with config (call on startup)"""
        if self._config.settings.auto_start:
